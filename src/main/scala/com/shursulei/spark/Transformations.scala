...
object Transformations {
  ...
  def mapTransformation(sc: SparkContext){
    ...
    //map适用于任何类型的元素，且对其作用的集合中的每一个元素循环遍历并调用其作为参数的函数对
    // 每一个遍历的元素进行具体化处理
  ...
  def filterTransformation(sc: SparkContext){
    ...
    //根据filter中作为参数的函数的Bool值来判断符合条件的元素，
    // 并基于这些元素构成新的MapPartitionsRDD
  ...
  def flatMapTransformation(sc: SparkContext){
    ...
    //parallelize是将字符串数组转化成RDD，创建以字符串为元素类型的parallelCollectionRDD
    val words = bigDataStrings.flatMap(_.split(" "))
    //首先是通过传入的作为参数的函数来作用于RDD的每个字符串进行单词切分（是以集合的方式存在的），
    // 然后把切分后的结果合并为一个大的集合产生结果为{Scala, Spark, Java, Hadoop, Java, Tachyon}
  ...
  def reduceByKeyTransformation(sc: SparkContext){
    ...
    /**
      ...
      */
    //对每一行的字符串进行单词拆分，并把所有行的拆分结果通过flat合并成一个大的单词集合
